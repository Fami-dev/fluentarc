...
Creator.AddSignal(Minimizer.InputBegan, function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
        ...
        startPosition = Minimizer.Position
        ...
        inputChangedConnection = UserInputService.InputChanged:Connect(function(changedInput)
            if isDragging and (changedInput.UserInputType == Enum.UserInputType.MouseMovement or changedInput.UserInputType == Enum.UserInputType.Touch) then
                ...
                local newX = startPosition.X.Scale + (delta.X / viewportSize.X)
                local newY = startPosition.Y.Scale + (delta.Y / viewportSize.Y)
                
                local elementSize = Minimizer.AbsoluteSize
                local maxX = (viewportSize.X - elementSize.X) / viewportSize.X
                local maxY = (viewportSize.Y - elementSize.Y) / viewportSize.Y

                Minimizer.Position = UDim2.fromScale(
